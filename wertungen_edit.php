...
// Prüfe, ob eine WertungID per GET übergeben wurde
if (!isset($_GET['id'])) {
    die('Keine WertungID angegeben.');
}
$wertungID = $_GET['id'];

// Vorhandenen Eintrag laden
$stmt = $pdo->prepare("SELECT * FROM Wertungen WHERE WertungID = :wertungID");
...
// Formularverarbeitung für die Bearbeitung
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($duplicateCount > 0) {
        ...
        // Aktualisiere den Datensatz, um die neuen Werte anzuzeigen
        $stmt = $pdo->prepare("SELECT * FROM Wertungen WHERE WertungID = :wertungID");
        $stmt->execute([':wertungID' => $wertungID]);
        $wertung = $stmt->fetch(PDO::FETCH_ASSOC);
    }
...
<div class="container mt-3">
    ...
    <form method="post">
        ...
        <div class="form-group">
            ...
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="P_Stufe" name="P_Stufe" value="<?php echo $wertung['P-Stufe']; ?>" placeholder="NULL">
        </div>
        <div class="form-group">
            <label for="D_Note">D-Note</label>
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="D_Note" name="D_Note" value="<?php echo $wertung['D-Note']; ?>" required>
        </div>
        <div class="form-group">
            <label for="E1_Note">E1-Note</label>
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="E1_Note" name="E1_Note" value="<?php echo $wertung['E1-Note']; ?>" placeholder="NULL">
        </div>
        <div class="form-group">
            <label for="E2_Note">E2-Note</label>
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="E2_Note" name="E2_Note" value="<?php echo $wertung['E2-Note']; ?>" placeholder="NULL">
        </div>
        <div class="form-group">
            <label for="E3_Note">E3-Note</label>
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="E3_Note" name="E3_Note" value="<?php echo $wertung['E3-Note']; ?>" placeholder="NULL">
        </div>
        <div class="form-group">
            <label for="E4_Note">E4-Note</label>
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="E4_Note" name="E4_Note" value="<?php echo $wertung['E4-Note']; ?>" placeholder="NULL">
        </div>
        <div class="form-group">
            <label for="nA_Abzug">nA-Abzug</label>
            <input type="number" step="0.01" inputmode="decimal" class="form-control" id="nA_Abzug" name="nA_Abzug" value="<?php echo $wertung['nA-Abzug']; ?>" required>
        </div>
